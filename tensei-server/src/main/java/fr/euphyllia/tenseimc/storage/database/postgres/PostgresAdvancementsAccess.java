...
public class PostgresAdvancementsAccess {

    private PostgresAdvancementsAccess() {
    }

    private static String upsertSql() {
        return """
                INSERT INTO %s.player_advancements (uuid, data, updated_at)
                VALUES (?, ?::jsonb, NOW())
    ...
    private static String selectSql() {
        return "SELECT data FROM %s.player_advancements WHERE uuid = ?".formatted(PostgresManager.schema());
    }
    ...
    public static void save(final UUID uuid, final JsonElement data) {
        ...
        try {
            ...
            throw new PermanentStorageException("Failed to wrap JSON for advancements " + uuid, ex);
        }

        SQLExecute.update(
                "save advancements for " + uuid,
                upsertSql(),
        ...
        return Optional.ofNullable(SQLExecute.queryMapOnLogin(
                ...
                rs -> {
                    try {
                        ...
                        throw new PermanentStorageException("Failed to read advancements for " + uuid, ex);
                    } catch (final Exception ex) {
                        throw new PermanentStorageException("Corrupted advancements JSON for " + uuid, ex);
                    }
                }
        ));
    }
